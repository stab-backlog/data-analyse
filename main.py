...
"""
 ...
 Есть ли взаимосвязь между рекламным каналом и использованием промокода?
 """
...
cross_tab = pd.crosstab(df['channel'], df['promo_code'])
_, p_value, _, _ = stats.chi2_contingency(cross_tab)
rounded = round(p_value, 3)
print(f'p-value:{rounded} & {rounded <= 0.05}')
